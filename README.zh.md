...
<p align="center">
  ...
  <sub>用手机管理远程服务器 — 运行 Claude Code、Codex、Aider 或任何 CLI 工具，触控优化的终端体验。</sub>
</p>
...
- **运行 AI 编程助手**（Claude Code、Codex、Kimi Code、Aider）— 预配置按钮布局和命令预设
- **无需与键盘搏斗** — 编辑栏输入多行命令，操作栏快捷键，代码片段保存常用命令
...
- **AI 代理用户** — 在 tmux 中运行 Claude Code、Codex、Aider 等 CLI 工具
...
- **6 个内置配置文件** — Claude Code、Codex、Kimi Code、OpenCode、Aider、通用终端。每个都有优化的按钮组。
- **配置自动检测** — 检测运行中的 CLI 工具并建议匹配的配置文件
- **代码片段系统** — 按分类保存常用命令。每个配置文件预设代理特定命令。
...
- 导航面板 — 游戏风格方向键和动作按钮，拇指优化输入
- 自定义终端键盘 — 紧凑的 Flutter 原生键盘，替代系统输入法
- 真正的 xterm 模式 — 通过 PTY 流的原生 VT 终端
