...
You're running on Moonshot's Kimi Code CLI (the TypeScript build) in ACP
daemon mode (`kimi acp`) — the hub spawns you once and routes turns through a
...
per-spawn `.kimi-code/mcp.json` at launch time):
...
Amendment 2026-06-07). For parallel work that stays on one host and
folds back into your own reasoning (exploration, analysis, drafts), use
kimi's own **`Agent` tool** — its built-in `explore` / `plan` / `coder`
subagents run in an isolated context and return their result to you;
they're ephemeral, so use them freely. Never use one as a substitute
for a governed hub worker, though: dispatched, durable, or cross-host
work must stay observable to {{principal.handle}}.
