...
func buildNativeTools() []nativeTool {
	return []nativeTool{
		...
		{
			Name:        "reference_list",
			Short:       "List references in the team's library.",
			Description: "List references from the hub reference library (papers, reports, notes), newest first. Optional filters: collection (name), tag, source (e.g. zotero), q (substring over title/authors/abstract), limit.",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
			...
			Tier: TierTrivial, WorkerEligible: true,
			Handler: teamArgs((*Server).mcpReferenceList),
		},
		{
			Name:        "reference_get",
			Short:       "Fetch one reference by id.",
			Description: "Fetch a single reference (all fields) by its id.",
			InputSchema: map[string]any{
				"type":       "object",
			...
			Tier: TierTrivial, WorkerEligible: true,
			Handler: teamArgs((*Server).mcpReferenceGet),
		},
		{
			...
			Tier: TierRoutine, WorkerEligible: true,
			Handler: teamArgs((*Server).mcpReferenceCreate),
		},
		{
			Name:        "reference_update",
			Short:       "Edit a reference by id.",
			Description: "Update a reference. Requires id; any other field present replaces its stored value, absent fields are kept. Same field set as reference_create.",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
			...
			Tier: TierRoutine, WorkerEligible: true,
			Handler: teamArgs((*Server).mcpReferenceUpdate),
		},
		{
			Name:        "reference_delete",
			Short:       "Remove a reference from the library.",
			...
			Tier: TierRoutine, WorkerEligible: true,
			Handler: teamArgs((*Server).mcpReferenceDelete),
		},
		{
			Name:        "reference_annotation_list",
			Short:       "List a reference's PDF annotations.",
			Description: "List the annotations (highlights, underlines, notes, area/ink) on one reference's PDF, ordered by page then reading position. Requires reference_id.",
			InputSchema: map[string]any{
				"type":       "object",
				"properties": map[string]any{"reference_id": map[string]any{"type": "string"}},
				"required":   []string{"reference_id"},
			},
			Tier: TierTrivial, WorkerEligible: true,
			Handler: teamArgs((*Server).mcpAnnotationList),
		},
		{
			Name:  "reference_annotation_create",
			Short: "Add a PDF annotation to a reference.",
			Description: "Create an annotation on a reference's PDF. Requires reference_id. type is highlight|underline|note|text|image|ink (default highlight). " +
				"position is the geometry as Zotero-shaped JSON in unscaled PDF points, origin bottom-left: " +
				`{"pageIndex":6,"rects":[[x1,y1,x2,y2]]} for rect kinds, or {"pageIndex":2,"paths":[[x1,y1,x2,y2]],"width":2} for ink. ` +
				"Other fields: color (hex), page_index (0-based, keep in step with position.pageIndex), comment, text (selected text), tags, author, sort_index.",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
					...
					"sort_index":   map[string]any{"type": "string"},
					"tags":         map[string]any{"type": "array", "items": map[string]any{"type": "string"}},
				},
				"required": []string{"reference_id"},
			},
			Tier: TierRoutine, WorkerEligible: true,
			Handler: teamArgs((*Server).mcpAnnotationCreate),
		},
		{
			Name:        "reference_annotation_update",
			Short:       "Edit a PDF annotation.",
			Description: "Update an annotation. Requires reference_id and id; any other field present replaces its stored value, absent fields are kept. Same field set as reference_annotation_create.",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"reference_id": map[string]any{"type": "string"},
				...
				"required": []string{"reference_id", "id"},
			},
			Tier: TierRoutine, WorkerEligible: true,
			Handler: teamArgs((*Server).mcpAnnotationUpdate),
		},
		{
			Name:        "reference_annotation_delete",
			Short:       "Remove a PDF annotation.",
			Description: "Delete an annotation by id. Requires reference_id and id.",
			InputSchema: map[string]any{
				"type": "object",
				"properties": map[string]any{
					"reference_id": map[string]any{"type": "string"},
			...
			Tier: TierRoutine, WorkerEligible: true,
			Handler: teamArgs((*Server).mcpAnnotationDelete),
		},
		{
...
var nativeToolMeta = map[string]struct {
	...
	"reference_list":              {true, []string{"reference_get", "search"}},
	"reference_get":               {true, []string{"reference_list", "reference_update"}},
	"reference_create":            {false, []string{"reference_update", "reference_list"}},
	"reference_update":            {false, []string{"reference_get", "reference_create"}},
	"reference_delete":            {false, []string{"reference_list"}},
	"reference_annotation_list":   {true, []string{"reference_get", "reference_annotation_create"}},
	"reference_annotation_create": {false, []string{"reference_annotation_update", "reference_annotation_list"}},
	"reference_annotation_update": {false, []string{"reference_annotation_list", "reference_annotation_create"}},
	"reference_annotation_delete": {false, []string{"reference_annotation_list"}},
