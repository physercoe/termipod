...
func buildTools() []toolDef {
	tools := []toolDef{
		...
		},
		{
			...
			Description: "Append a step to a plan. Requires `plan`, `phase_idx`, `step_idx`, `kind` (one of agent_spawn|llm_call|shell|mcp_call|human_decision). `spec_json` is REQUIRED for every kind except `human_decision` and carries the kind-specific fields the executor needs.\n\n=== spec_json shape per kind ===\n\n  - `agent_spawn`: `{child_handle: string, template?: string, spawn_spec_yaml?: string, ...}` — minimum `child_handle`. See agents.spawn for full spec.\n  - `llm_call`: `{prompt: string, model?: string, temperature?: number, max_tokens?: integer}` — minimum `prompt`.\n  - `shell`: `{cmd: string, cwd?: string, timeout_sec?: integer}` — minimum `cmd`.\n  - `mcp_call`: `{tool: string, args?: object}` — minimum `tool`.\n  - `human_decision`: optional `{prompt?: string}` — the step's own copy serves as the prompt when spec_json is empty.\n\nv1.0.620+ enforces these required fields with HTTP 422 at the verb boundary; pre-v1.0.620, under-specified specs were silently inserted and the plan stalled at execution. See docs/discussions/validate-at-every-boundary.md §3 Layer 1.\n\nExample (agent_spawn step):\n```json\n{\n  \"plan\": \"01J...\",\n  \"phase_idx\": 2,\n  \"step_idx\": 0,\n  \"kind\": \"agent_spawn\",\n  \"spec_json\": {\n    \"child_handle\": \"ml-worker-1\",\n    \"template\": \"ml-worker.v1\"\n  }\n}\n```",
		...
		{
			Name:        "agents.spawn",
			Description: "Spawn a child agent. Requires `child_handle`, `kind`, `spawn_spec_yaml`. Optional: host_id, parent_agent_id, worktree_path, budget_cents, mode, project_id (binds the agent to a project per ADR-025; the YAML `project_id:` is the canonical site, this body field is a fallback), permission_mode (`skip` = auto-allow tool calls via `--dangerously-skip-permissions`; `prompt` = route every tool call through the hub permission_prompt MCP gate; omitted → `skip` so worker tools work out of the box, matching the mobile bootstrap default), wait (default true: sync-wait three-state return — see below), wait_seconds (cap, default 30, max 50). May return 202 + attention_id if policy gates the spawn on approval. Project-bound spawns require the caller to be that project's steward (ADR-025 W9); the general steward must delegate via `request_project_steward`.\n\n=== spawn_spec_yaml shape ===\n\nThe spec is a YAML document describing the engine to launch and (optionally) the workdir, persona, and per-call gate. Two valid shapes:\n\n(a) Template-reference shorthand (recommended for stewards using bundled engines):\n```yaml\ntemplate: coder.v1\n```\nThe hub loads `hub/templates/agents/coder.v1.yaml` (or the matching user overlay at `team/templates/agents/`) and merges its full backend block into the spec. Your spec's fields override the template's via deep-merge (e.g. set `backend:\\n  model: claude-haiku-4-5` on top of a template reference to override only the model). The lookup accepts EITHER the file basename (`coder.v1`) OR the internal `template:` field value (`agents.coder`). v1.0.620+: template references are merged before {{var}} expansion; pre-v1.0.620 a bare `template: coder.v1` silently failed.\n\n(b) Inline shape (when no template fits):\n```yaml\nbackend:\n  cmd: claude --print --output-format stream-json --input-format stream-json --verbose --dangerously-skip-permissions\n  kind: claude-code\n  model: claude-opus-4-7\nprompt: coder.v1.md\nproject_id: 01K...\n```\n\nMINIMUM viable spec: `backend.cmd` must be non-empty. v1.0.620+ enforces this with HTTP 422 at the hub boundary; under-specified specs are no longer silently launched as bash placeholders. See docs/discussions/validate-at-every-boundary.md for the principle.\n\n=== Sync-wait three-state return (v1.0.620+) ===\n\nThe MCP path defaults to `wait: true`. The response's `status` field reflects the new agent's real lifecycle state:\n\n  - `running` — engine started successfully within wait_seconds.\n  - `failed` — hostrunner refused or engine crashed; `failure_reason` is populated.\n  - `pending` — wait_seconds expired without state transition; poll `agents.get` to learn the final state.\n\nSet `wait: false` for the legacy async return (`status: 'spawned'` immediately, no engine confirmation). The wait cap is hard-limited at 50 seconds to stay below Claude Code's 60-second MCP_TOOL_TIMEOUT with margin for transport latency.\n\n=== ADR-029 task linkage ===\n\nLink the spawn to a task. Pass `task_id` to attach to an existing task (flips status to 'in_progress' and stamps started_at if not already running; 409 if task is 'done' or 'cancelled' — call tasks.update status='in_progress' first to reopen). Or pass `task` (object with title, body_md, parent_task_id, milestone_id, priority) to materialize a fresh task in the same transaction, assignee = the new agent, created_by = parent_agent_id. `task_id` and `task` are mutually exclusive (400).\n\n=== Worker delivery ===\n\nThe linked task's title + body_md are inlined into the worker's agent-memory file (CLAUDE.md for claude-code, AGENTS.md for codex/kimi, GEMINI.md for gemini-cli) under a `## Task` section before launch, so the worker reads the instructions on first turn — no follow-up `a2a.invoke` is needed just to deliver the initial work. (You may still `a2a.invoke` later for clarifications or to push new context.)",
			InputSchema: schema(`{"type":"object","required":["child_handle","kind","spawn_spec_yaml"],"properties":{"child_handle":{"type":"string"},"kind":{"type":"string"},"spawn_spec_yaml":{"type":"string"},"host_id":{"type":"string"},"parent_agent_id":{"type":"string"},"worktree_path":{"type":"string"},"budget_cents":{"type":"integer"},"mode":{"type":"string"},"project_id":{"type":"string"},"permission_mode":{"type":"string","enum":["skip","prompt"],"description":"omit for worker spawns (default skip); set to 'prompt' to route every tool call through the hub permission_prompt MCP gate (creates attention_items the principal approves)"},"task_id":{"type":"string"},"task":{"type":"object","properties":{"title":{"type":"string"},"body_md":{"type":"string"},"parent_task_id":{"type":"string"},"milestone_id":{"type":"string"},"priority":{"type":"string","enum":["low","med","high","urgent"]}},"required":["title"]},"wait":{"type":"boolean","description":"Default true on the MCP path: block until the agent reaches running or failed, bounded by wait_seconds. Response status reflects real engine state. Set false to return immediately with status='spawned' (legacy async behaviour)."},"wait_seconds":{"type":"integer","description":"Cap on the wait window. Default 30, hard-capped at 50 (below Claude Code's 60s MCP_TOOL_TIMEOUT)."}}}`),
			call: func(c *hubClient, args map[string]any) (any, error) {
				...
				// for the misleading "spawned" return that motivated
				// this. wait_seconds cap (50s) is enforced server-side.
				if _, set := args["wait"]; !set {
					args["wait"] = true
				}
				var out json.RawMessage
		...
		},
		{
			...
			call: func(c *hubClient, args map[string]any) (any, error) {
				...
				if err := c.do("DELETE", c.teamPath("/projects/"+url.PathEscape(p)+"/tasks/"+url.PathEscape(id)), nil, nil, nil); err != nil {
					return nil, err
				}
				return map[string]any{"ok": true, "project_id": p, "task": id}, nil
			},
