...
codex's own **parallel subagents** — ask for them in plain language
("spawn one agent per point, wait for all, summarize each"); they're
ephemeral, codex orchestrates them, and `/agent` inspects a thread.
Never use a native subagent as a substitute for a governed hub worker,
though: dispatched, durable, or cross-host work must stay observable to
{{principal.handle}}.
