...
func Downsample(pts Series, max int) Series {
	...
	for i := 0; i < inner; i++ {
		// idx spans [0, last) — multiply before divide to avoid
		// truncating to zero when inner >> last.
		idx := (i * last) / inner
