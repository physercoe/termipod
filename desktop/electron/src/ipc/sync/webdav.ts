...
///
/// PROXY: the `proxy` arg is honoured — every request goes through `proxyFetch`,
/// which routes via an undici ProxyAgent when a proxy is configured and a direct
/// fetch otherwise.
...
async function propfindDir(
  ...
  const resp = await dav(childUrl(base, dirRel, true), 'PROPFIND', auth, proxy, {
...
async function enumerateRemote(base: URL, auth: string, proxy: string | undefined): Promise<Map<string, RemoteFile>> {
  ...
  while (queue.length > 0) {
    ...
    await propfindDir(base, basePath, dirRel, auth, proxy, files, subdirs);
...
async function mkcolParents(
  ...
  for (const part of parts.slice(0, -1)) {
    ...
    const resp = await dav(childUrl(base, acc, true), 'MKCOL', auth, proxy);
    const s = resp.status;
...
async function upload(
  ...
  await mkcolParents(base, rel, auth, proxy, made);
  const bytes = await readFile(local.abs);
  const resp = await dav(childUrl(base, rel, false), 'PUT', auth, proxy, {
...
async function download(base: URL, rel: string, root: string, auth: string, proxy: string | undefined): Promise<void> {
  const resp = await dav(childUrl(base, rel, false), 'GET', auth, proxy);
...
export const folderWebdavHandlers: Record<string, Handler> = {
  folder_webdav_verify: async (args): Promise<string> => {
    ...
    const resp = await dav(base.href, 'PROPFIND', auth, proxy, {
  ...
  folder_webdav_sync: async (args, ctx): Promise<FolderSyncReport> => {
    ...
    const remotes = await enumerateRemote(base, auth, proxy);
