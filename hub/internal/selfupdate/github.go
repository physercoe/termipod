...
// — "stable" skips prereleases, "alpha" includes them. GitHub returns
// the list newest-first, so the first match is the latest.
func (c *ghClient) resolveRelease(ctx context.Context, version, channel string) (*ghRelease, error) {
	if version != "" {
	...
	case "", "stable":
		...
		return nil, fmt.Errorf("no stable release in %s — the project currently ships "+
			"only -alpha tags; pass --channel alpha or an explicit --version", c.repo)
	case "alpha":
		for i := range rels {
			if !rels[i].Draft {
				return &rels[i], nil
		...
		return nil, fmt.Errorf("no published release in %s", c.repo)
	default:
