...
| **Steward** | An agent. CEO-class operator that runs the system on the principal's behalf. Two tiers (general + domain) per [ADR-017](../decisions/017-layered-stewards.md); project domain stewards are lazy-materialized with director consent per [ADR-025](../decisions/025-project-steward-accountability.md). | Broad — manager scope. Authors templates and schedules, spawns workers, drives plans, surfaces decisions to the director. *Does not* do IC work (manager/IC invariant). Spawn authority is tier-bounded: general routes intent, project steward owns its project's worker spawns ([ADR-025 D3](../decisions/025-project-steward-accountability.md)). | Persistent (general) or project-scoped (domain). |
| **Worker** | An agent. IC-class executor. Project-scoped ([ADR-025 D1](../decisions/025-project-steward-accountability.md)) — every worker carries `project_id` and has its own session for observability. | Bounded — IC scope (per-template `tool_allowlist` + ADR-016 worker role). Reads, writes, runs experiments, produces artifacts. *Does not* spawn peers or author templates. Only spawnable by its project's steward. | Project-scoped or task-scoped; archives at completion. |
...
- [ADR-025 — project steward accountability](../decisions/025-project-steward-accountability.md) — lazy materialization, director consent, worker project-binding.
