...
/// subscription stay within the project's blast radius. Backfill via
/// REST, live updates via SSE, composer posts `message` events with
/// optional attachment parts.
class ProjectChannelScreen extends ConsumerStatefulWidget {
  final String projectId;
  final String channelId;
  final String channelName;
  const ProjectChannelScreen({
    super.key,
  ...
  @override
  ConsumerState<ProjectChannelScreen> createState() =>
      _ProjectChannelScreenState();
}

class _ProjectChannelScreenState extends ConsumerState<ProjectChannelScreen> {
  ...
  Future<void> _bootstrap() async {
    ...
    try {
      ...
      _jumpToBottom();
      _sub = client
          .streamEvents(widget.projectId, widget.channelId)
          .listen(
        (evt) {
          if (!mounted) return;
  ...
  Future<void> _send() async {
    ...
    try {
      await client.postProjectChannelEvent(
        widget.projectId,
        widget.channelId,
        type: 'message',
        parts: [
