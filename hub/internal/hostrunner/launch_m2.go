...
// writeKimiTSMCPConfig emits kimi's JSON form at
// <workdir>/.kimi-code/mcp.json (ADR-054 D3). The engine
// auto-discovers mcp.json at two levels — user
...
func writeKimiTSMCPConfig(workdir, hubURL, token string) error {
	...
	// seed; parse failure → fail loud — we don't silently clobber a
	// file the operator may rely on for non-termipod MCP servers).
