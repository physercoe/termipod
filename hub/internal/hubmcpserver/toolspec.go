...
func toolRegistry() []ToolSpec {
	...
	specs := []ToolSpec{
		...
		spec("runs_get",
			...
			tierTrivial, true),
		spec("run_metrics",
			"Fetch one run's metric curves — step-indexed points plus last_value per metric. Required: run.",
			tierTrivial, true),
		spec("run_config_diff",
			"Compare 2-8 runs' configs and return only what differs, one cell per run. Required: runs.",
			tierTrivial, true),
		spec("runs_create",
