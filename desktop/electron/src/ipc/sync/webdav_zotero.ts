...
async function req(
  ...
  return proxyFetch(url, { method, headers, body: init?.body, signal: AbortSignal.timeout(DAV_TIMEOUT_MS) }, proxy);
}

async function mkcol(dav: string, auth: string, proxy: string | undefined): Promise<void> {
  const s = (await req(dav, 'MKCOL', auth, proxy)).status;
...
async function put(url: string, body: Uint8Array, contentType: string, auth: string, proxy: string | undefined): Promise<void> {
  const resp = await req(url, 'PUT', auth, proxy, { body, contentType });
...
async function getProp(
  ...
  const resp = await req(`${dav}${key}.prop`, 'GET', auth, proxy);
  const s = resp.status;
...
async function upload(dav: string, key: string, local: LocalAtt, auth: string, proxy: string | undefined): Promise<void> {
  const zipped = await zipFiles(local.files);
  await put(`${dav}${key}.zip`, zipped, 'application/zip', auth, proxy);
  // Prop last — its presence marks a completed upload (Zotero's rule).
  await put(`${dav}${key}.prop`, Buffer.from(buildProp(local.mtimeMs, local.hash)), 'text/xml; charset=utf-8', auth, proxy);
}

/// Download + extract `<KEY>.zip` into `root/<KEY>/`; false when the zip is
/// missing (404). Mirrors webdav.rs `download`.
async function download(dav: string, key: string, root: string, auth: string, proxy: string | undefined): Promise<boolean> {
  const resp = await req(`${dav}${key}.zip`, 'GET', auth, proxy);
  const s = resp.status;
...
export const webdavZoteroHandlers: Record<string, Handler> = {
  webdav_verify: async (args): Promise<string> => {
    ...
    await mkcol(dav, auth, proxy);
    // Prove write access with a probe file.
    await put(`${dav}lastsync.txt`, Buffer.from(String(Date.now())), 'text/plain', auth, proxy);
    return 'ok';
  },
