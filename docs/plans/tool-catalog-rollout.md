...
> **Type:** plan
> **Status:** Done (2026-05-18) — seven wedges, three phases (W4 split). Every MVP tool is in the unified `ToolSpec` registry: 66 authority-backed tools (`hubmcpserver`'s registry) + 26 native tools (the `server`-side native registry). All carry `snake_case` canonical names, dotted names kept as deprecated aliases; catalog, tier, and worker role-eligibility derive from the spec; CI-lock tests guard both registries. W3 closed the security gotcha — `dispatchTool` resolves the canonical name before the `agents_spawn` / `a2a_invoke` literal-name gates. W4n built the native-dispatch path. D-4 consolidated all three duplicate pairs (`list_agents`→`agents_list`, `get_audit`→`audit_read`, `get_task`→`tasks_get` — now deprecated aliases; `get_task`'s bare-`id` contract preserved by a new team-scoped `GET /teams/{team}/tasks/{id}` endpoint). **W6 tail shipped** — see [tool-catalog-w6-teardown](tool-catalog-w6-teardown.md): `buildNativeTools()` is now the single native-tool declaration, the legacy `mcpToolDefsBase/Extra/orchestrationToolDefs` are deleted, dead handlers removed, bundled templates swept onto canonical names (+ a drift-lock), and the standalone daemon's catalog aligned to the registry. `dispatchTool` routes no tool by a literal `case`. **ADR-033 is complete.** Implements [ADR-033](../decisions/033-tool-catalog-naming-and-registration.md); the decision rationale and the catalog audit are there and in the [tool-catalog-structure discussion](../discussions/tool-catalog-structure.md).
...
**Re-verified against code (2026-05-18).** The discussion §2.2
audit had drifted: the actual handlers show the *authority* tool in
each pair is already a superset of — or equivalent to — its native
twin, so **no field-union merge is needed**.

- **`agents_list`** ← `list_agents` *(shipped).* `handleListAgents`
  already returns `pane_id` (`agentOut.PaneID`, `omitempty`) and
  every richer field; `agents_list` is a strict superset.
  `list_agents` is now a deprecated alias of `agents_list`.
- **`audit_read`** ← `get_audit` *(shipped).* `handleListAudit`
  already supports the `action` filter and caps `limit` at 500 —
  identical to native `get_audit`. The `audit.read` MCP adapter
