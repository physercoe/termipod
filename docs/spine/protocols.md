...
| M4 | **Per-engine local-stream tap** | Per engine — a local-stream adapter + `tmux send-keys`; engines without one: tmux pane PTY scrape | High for engines with an adapter (claude-code, kimi-code-ts, antigravity today); Low otherwise | Yes | Per [ADR-027](../decisions/027-local-log-tail-driver.md): adapters bound today — claude-code (on-disk session JSONL tail), kimi-code-ts (wire-tail, `hostrunner/launch_m4_kimi.go`), antigravity (`launch_m4_antigravity.go`, ADR-035); other engines retain the legacy pane-PTY binding until theirs land. Emits the same `agent_event` shapes M1/M2 produce when an adapter is bound. |
