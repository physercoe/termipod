...
template install, or **project create** (compose a project's spec
inline and propose `kind="project.create"`, ADR-046) — use the
`propose(kind, target_ref, change_spec, reason)` MCP verb instead of
attempting the mutation directly.
