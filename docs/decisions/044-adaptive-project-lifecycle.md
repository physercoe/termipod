...
The phased rollout (P1 reads + direct materialization, P2 `criteria.*` +
`deliverable.create` propose verbs, P3 AC-driven auto-advance — all
shipped, with file-level detail) lives in the plan,
[`plans/adaptive-project-lifecycle.md`](../plans/adaptive-project-lifecycle.md).
This ADR records the decisions; the plan owns the *how* and the status.
