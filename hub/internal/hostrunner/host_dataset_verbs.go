...
//
// Scope of what these expose: datasetmeta opens files under <root>/meta/ only,
// and its Source refuses any name that escapes the root. The verbs therefore
// cannot return arbitrary host files — the most a caller learns about a path it
// guesses is whether a LeRobot dataset sits there. Digests and episode
// listings are summaries; frames, parquet data shards and mp4s are never read
// and never travel.
