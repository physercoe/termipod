...
func buildTools() []toolDef {
	tools := []toolDef{
		...
		{
			Name:        "runs_create",
			Description: "Create a new run row. Requires `project_id`. Optional: `agent_id`, `config_json` (object), `seed`, `parent_run_id`, `trackio_host_id`, `trackio_run_uri`, `env_ref` (the environment this run ran in — opaque `family:env_id@version`, e.g. `maniskill:PickCube-v1@0.6`; unvalidated, and left empty when you don't know one).\n\nTo surface training curves on mobile, set `trackio_run_uri` to `trackio://<project>/<run_name>` (the `project=`/`name=` you pass to `trackio.init`; the project's SQLite is `<TRACKIO_DIR>/<project>.db`, default `~/.cache/huggingface/trackio`). `trackio_host_id` is the host where the worker logs — OPTIONAL when `agent_id` is set (the hub fills it from that agent's host). If you don't know the run name yet at creation, link it later with `runs_update`. Typos are fixable with `runs_update` — no need to recreate the run.",
			InputSchema: schema(`{"type":"object","required":["project_id"],"properties":{"project_id":{"type":"string"},"agent_id":{"type":"string"},"config_json":{"type":"object"},"seed":{"type":"integer"},"parent_run_id":{"type":"string"},"trackio_host_id":{"type":"string"},"trackio_run_uri":{"type":"string"},"env_ref":{"type":"string"}}}`),
			call: func(c *hubClient, args map[string]any) (any, error) {
		...
		},
		{
			...
			call: func(c *hubClient, args map[string]any) (any, error) {
				...
				// Bare-handle convention: storage doesn't include `@`.
				// Tolerate callers that prefix one (or, defensively,
				// two — the post-v1.0.636 `@@parent.handle` bug class)
				// so a stochastic LLM that re-adds the display sigil
				// still resolves. The lookup itself is exact-match;
				// normalize before sending.
				for strings.HasPrefix(handle, "@") {
