...
| [064](064-agent-desktop-coworking-contract.md) | **Agent desktop co-working contract** — the agent is a **first-class user of the local-work tabs** (Read, Author, Inspect, Compare, Replay, Record) by operating on the **entities each tab renders + pointing + a consented `navigate` verb — never synthetic input** (ADR-062's "the click is the user's" stands; Fleet/Projects stay hub-native; Terminal/Settings/Vault structurally excluded); **tools live where the entity lives** (desktop-owned → frozen bridge surfaces like `author_read`/`author_apply`/`author_render`/`author_guide`; hub-owned → the ADR-033 catalog; never both); consent = the four-class ladder **`read | annotate | navigate | write`** where desktop-entity writes take an **approval card granting a per-(agent, target, session) lease — never bearer scope** (read-pinned kimiweb participates) and hub-entity writes keep hub governance; the **like-a-user apply contract** — validated-before-commit (atomic; malformed applies must never degrade a document), landing live via imperative adapters, undoable, attributed, revertible, degradation stated honestly — plus **write-not-blind**: a surface's focus fields ship before its write verbs; native grammars only, vendored Apache-2.0 logic/data under NOTICE | Proposed 2026-08-01 | entity mutation is a governed action, not UI actuation — composes with [062](062-desktop-ui-as-agent-addressable-entity.md); verbs per [030](030-governed-actions-and-propose-verb.md); bridge gates per [059](059-agent-browser-bridge.md); metadata per [063](063-mcp-version-negotiation-and-adoption.md) D5; library entity per [053](053-hub-reference-library-entity.md); fed by [agent-desktop-coworking](../discussions/agent-desktop-coworking.md); executed by [agent-desktop-coworking](../plans/agent-desktop-coworking.md) + [desktop-compare-wall-and-decisions](../plans/desktop-compare-wall-and-decisions.md) |
