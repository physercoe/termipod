...
// TestInit_RetiredTemplatesAreGone confirms the legacy templates that
// were retired in v1.0.507 (ablation-sweep + benchmark-comparison)
// no longer get seeded by Init. Regression guard: re-introducing them
// should fail this test.
func TestInit_RetiredTemplatesAreGone(t *testing.T) {
	dir := t.TempDir()
	...
	defer db.Close()
	for _, name := range []string{"ablation-sweep", "benchmark-comparison"} {
		var count int
		...
		if count != 0 {
			t.Errorf("%s template row count = %d, want 0 (retired in v1.0.507)", name, count)
		}
	}
}
