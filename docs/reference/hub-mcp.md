...
| Tool | Purpose |
|---|---|
| `agents.spawn` | Spawn a child agent. **`child_handle`**, **`kind`**, **`spawn_spec_yaml`**; optional `host_id`, `parent_agent_id`, `worktree_path`, `budget_cents`, `mode`. May return 202 + attention_id when policy gates the spawn. |
| `agents.list` | List agents. Optional `host_id`, `status` filters; `include_archived: true` to also show soft-deleted rows. |
